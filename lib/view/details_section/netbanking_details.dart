...
class NetBankingDetails extends StatelessWidget {
  final int _id;
  NetBankingDetails(this._id);
          ...
          : Scaffold(
              body: SafeArea(
                child: SingleChildScrollView(
                  ...
                  child: FutureBuilder(
                    future: _helper.getNetBankingById(_id),
                      ...
                      return Column(
                        ...
                        children: [
                          ...
                          GetListTile(
                              height,
                              'User ID',
                              Encryption.decryptText(snapshot.data['userid']),
              ...
              Get.defaultDialog(
                  ...
                  onConfirm: () async {
                    if (_formKey.currentState.validate()) {
                      ...
                      if (title == 'Bank Name') {
                        await helper.updateNetBankingBankName(
                            _id, _textEditingController.text.trim());
                        Get.back();
                        MySnackBar.getSnackBar(
                            'Success', '$title updated successfully.');
                      } else if (title == 'Bank URL') {
                        await helper.updateNetbankingUrl(
                            _id, _textEditingController.text.trim());
                        Get.back();
                        MySnackBar.getSnackBar(
                            'Success', '$title updated successfully.');
                      } else if (title == 'User ID') {
                        await helper.updateNetbankingUserid(
                            _id,
                        ...
                        MySnackBar.getSnackBar(
                            'Success', '$title updated successfully.');
                      } else if (title == 'Corporate ID') {
                        await helper.updateNetbankingCorporateId(
                            _id,
                            Encryption.encryptText(
                                _textEditingController.text.trim()));
                        Get.back();
                        MySnackBar.getSnackBar(
                            'Success', '$title updated successfully.');
                      } else if (title == 'Login Password') {
                        await helper.updateNetbankingLoginpass(
                            _id,
                            Encryption.encryptText(
                                _textEditingController.text.trim()));
                        Get.back();
                        MySnackBar.getSnackBar(
                            'Success', '$title updated successfully.');
                      } else if (title == 'Profile Password') {
                        await helper.updateNetbankingProfilepass(
                            _id,
                            Encryption.encryptText(
                                _textEditingController.text.trim()));
                        Get.back();
                        MySnackBar.getSnackBar(
                            'Success', '$title updated successfully.');
                      } else if (title == 'Transaction Password') {
                        var result =
                            await helper.updateNetbankingTransactionPass(
                                _id,
                                Encryption.encryptText(
                                    _textEditingController.text.trim()));
                        Get.back();
                        ...
                        await helper.updateNetbankingExtra(
                            _id,
                ...
                onPressed: () {
                  try {
                    ...
                    ex += titleController.text.trim() +
                        ...
                        ";";
                    DatabaseHelper helper = DatabaseHelper();
                    helper.updateNetbankingExtra(_id, ex);
                    Get.back();
                    MySnackBar.getSnackBar(
